<template>
  ...
  >
    ...
    >
      <div
        class="bp-bar_sub_active"
        :style="{
          width: barSubActiveWidth,
          height: barSubActiveHeight,
          backgroundColor: subActiveColor,
          top: subActiveTop,
          bottom: subActiveBottom,
          left: subActiveLeft,
          right: subActiveRight,
          borderRadius: isActiveCircular ? barBorderRadius : 0,
        }"
      />
      <div
        class="bp-bar_active"
    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- 进度值 -->
          <span
            v-if="handleImgUrl == '' && infoAlign == 'handle' && showInfo"
            class="bp-handle-text"
            :style="{
              fontSize: infoFontSize,
              color: infoColor,
              width: mhandleWidth,
            ...
            >{{ infoContent == "subValue" ? msubValue : showValue
            }}{{ infoEndText }}</span
          >
          <!-- 挂件 -->
          <!-- 图片挂件 -->
          <img
            v-if="widgetPos == 'top' && widgetUrl"
            class="bp-handle-widget"
            :src="widgetUrl"
            :style="{
              flexDirection: 'column',
              borderRadius: mwidgetBorderRadius,
              bottom: moffset,
              width: mwidgetWidth,
              height: mwidgetHeight,
              opacity: widgetOpacity,
              transform: mwidgetAngle,
            }"
          />
          <img
            v-if="widgetPos == 'right' && widgetUrl"
            class="bp-handle-widget"
            :src="widgetUrl"
            :style="{
              flexDirection: 'row',
          ...
          />
          <img
            v-if="widgetPos == 'bottom' && widgetUrl"
            class="bp-handle-widget"
            :src="widgetUrl"
            :style="{
              flexDirection: 'column',
    ...
    <span
      v-if="
        showValueState() ||
        (infoAlign == 'center' && direction != 'vertical' && showInfo)
      "
      class="bp-value"
      :style="{
        color: infoColor,
...
export default {
  ...
  computed: {
    ...
    mareaWidth() {
      ...
      const width = this.sizeDeal(this.width)[0];
    ...
    barMaxHeight() {
      if (this.direction == "vertical") {
        const width = this.sizeDeal(this.width)[0];
        const handleWidth = this.sizeDeal(this.handleWidth);
    ...
    barMaxLeft() {
      if (this.showValueState()) {
        if (this.infoAlign == "left") {
          return (
            this.textWidth() + this.sizeDeal(this.handleWidth)[0] / 2 + "px"
          );
        }
      }
      if (this.direction != "vertical") {
      ...
      // vertical
      return (
        (this.maxHeight()[0] - this.sizeDeal(this.strokeWidth)[0]) / 2 + "px"
      );
    },
    activeRight() {
      if (this.reverse) {
    ...
    barActiveWidth() {
      ...
      if (this.continuous) {
        ...
        percent = Math.abs(
          (this.showValue - this.min) / (this.mmax - this.min)
        );
      }
    ...
    barSubActiveWidth() {
      ...
      if (this.subContinuous) {
        ...
        return (
          this.barMaxLength *
            Math.abs((this.msubValue - this.min) / (this.mmax - this.min)) +
          "px"
        );
      }
    },
    ...
    mhandleTop() {
      if (this.direction == "vertical") {
        ...
        // 拖柄垂直居中
        const handle = this.sizeDeal(this.handleHeight)[0];
        const top = this.maxHeight()[0] / 2 - handle / 2 + "px";
        return top;
      }
    },
    showValue() {
    ...
    textHeight() {
      const infoSize = this.sizeDeal(this.infoFontSize);
    ...
    valueLeft() {
      if (this.infoAlign == "left") {
        ...
        const width = this.sizeDeal(this.width);
    ...
    moffset() {
      const off = this.sizeDeal(this.widgetOffset);
  ...
  watch: {
    ...
    showValue(newValue, oldValue) {
      ...
      if (!this.continuous) {
        ...
        if (this.reverse) {
          if (this.direction != "vertical") {
            percent = Math.abs(
              1 - (newValue - this.min) / (this.mmax - this.min)
            );
          ...
          if (this.direction != "vertical") {
            ...
            percent =
              1 - Math.abs((newValue - this.min) / (this.mmax - this.min));
            this.handleY = percent * this.barMaxLength;
          }
        }
      }
    ...
    subValue(newValue, oldValue) {
      ...
      this.subPercent = Math.abs(
        (newValue - this.min) / (this.mmax - this.min)
      );
      this.$emit("change", {
        bpname: this.bpname,
        type: "change",
    ...
    handleX(newValue, oldValue) {
      if (newValue != oldValue) {
        // const dom = document.getElementById('bp-mdiv');
        const dom = this.$refs.bpmdiv;
  ...
  created() {
    ...
    // this.px2rpx = 1980 / systemInfo.screenWidth
  ...
  mounted() {
    ...
    this.percent = Math.abs(
      (this.valueFormat(this.value) - this.min) / (this.mmax - this.min)
    );
    this.subPercent = Math.abs(
      (this.valueFormat(this.subValue, true) - this.min) /
        (this.mmax - this.min)
    );
    if (this.reverse) {
      if (this.direction != "vertical") {
  ...
  methods: {
    drag(e) {
      const left = this.$refs.bingProgress.getBoundingClientRect().left;
      const right = this.$refs.bingProgress.getBoundingClientRect().right;
      const oDiv = this.$refs.bingProgress;
      // 鼠标按下，计算当前元素距离可视区的距离
      ...
      oDiv.onmousemove = function (e) {
        ...
        // 移动当前元素
        if (e.clientX <= left + 5 || e.clientX >= right - 8) {
          return;
        }
        _this.handleMove({ pageX: e.clientX });
      };
      // 鼠标收起
    ...
    handleMove(detail) {
      ...
      if (this.direction != "vertical") {
        ...
        if (this.infoAlign == "left") {
          ...
          percent =
            (detail.pageX - this.mainInfo.left - handleWidth[0] / 2) /
            this.barMaxLength;
        }
      }
      // vertical 模式
        ...
        percent =
          1 - (detail.pageY - handleWidth[0] / 2 - 1) / this.barMaxLength;
      }
    ...
    updateRect() {
       ...
       * 非NVUE movable-area 滑动事件获取到的位置是相对于文档的，获取组件位置，用于计算滑块位置
       */
      // const query = document.getElementById('bingProgress');
      const query = this.$refs.bingProgress;
      const rect = query.getBoundingClientRect();
      this.mainInfo.top = rect.top;
      this.mainInfo.left = rect.left;
      this.mainInfo.bottom = rect.bottom;
      this.mainInfo.right = rect.right;
    },
    showValueState() {
      if (
        this.direction != "vertical" &&
        this.showInfo &&
        (this.infoAlign == "left" || this.infoAlign == "right")
      ) {
        return true;
      }
      return false;
    ...
    valueFormat(v, isSub) {
      ...
      const stepInfo = this.stepInfo(isSub);
      v = Number(v - this.min).toFixed(stepInfo[1]);
      const step = stepInfo[0] * 10 ** stepInfo[1];
      const valueE = v * 10 ** stepInfo[1];
      const remainder = valueE % step;
      const remainderInt = Math.floor(remainder);
      // 对余数四舍五入0-1
      const sub = Math.round(remainder / step);
      let value =
        (Math.floor(valueE) - remainderInt + sub * step) / 10 ** stepInfo[1];
    ...
    textWidth() {
      if (this.showValueState()) {
        const numWidth =
          this.mmax.toString().length > this.min.toString().length
            ? this.mmax.toString().length
            : this.min.toString().length;
        const textWidth =
          ((numWidth + this.stepInfo()[1]) * 0.7 + this.infoEndText.length) *
          this.sizeDeal(this.infoFontSize)[0];
